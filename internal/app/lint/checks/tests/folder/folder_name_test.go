// internal/app/lint/checks/tests/folder/folder_name_test.go
package tests
...
func runFolderNameCases(t *testing.T, ctx context.Context, cases []folderTestCase) {
	t.Helper()
...
func newFolderCase(cfg folderCaseConfig) folderTestCase {
	return folderTestCase{
		...
		checker: lint.WithSelector(
			cfg.selector,
			folder.NewFolderNameCheck(cfg.config, cfg.code, reporting.SevError),
		),
...
func predicateRuleFailsCase(code string) folderTestCase {
	return newFolderCase(folderCaseConfig{
		...
		selector: lint.Selector{PathRegexes: mustCompile(`^src/[^/]+$`), Kind: "folder"},
		config:   folder.FolderNameConfig{Predicates: []string{"kebab"}},
		code:     code,
		expects: []testutil.Expect{
			{Code: code, Msg: "name 'myComponent' does not satisfy predicate: kebab"},
		},
	})
}
...
func prefixRuleFailsCase(code string) folderTestCase {
	return newFolderCase(folderCaseConfig{
		...
		selector: lint.Selector{PathRegexes: mustCompile(`^src/[^/]+$`), Kind: "folder"},
		config:   folder.FolderNameConfig{Prefix: "comp-"},
		code:     code,
		expects: []testutil.Expect{
...
func suffixRuleFailsCase(code string) folderTestCase {
	return newFolderCase(folderCaseConfig{
		...
		selector: lint.Selector{PathRegexes: mustCompile(`^svc/[^/]+$`), Kind: "folder"},
		config:   folder.FolderNameConfig{Suffix: "-svc"},
		code:     code,
		expects: []testutil.Expect{
...
func prohibitPrefixCase(code string) folderTestCase {
	return newFolderCase(folderCaseConfig{
		...
		selector: lint.Selector{PathRegexes: mustCompile(`^src/[^/]+$`), Kind: "folder"},
		config:   folder.FolderNameConfig{ProhibitPrefix: "_"},
		code:     code,
		expects: []testutil.Expect{
...
func prohibitSuffixCase(code string) folderTestCase {
	return newFolderCase(folderCaseConfig{
		...
		selector: lint.Selector{PathRegexes: mustCompile(`^src/[^/]+$`), Kind: "folder"},
		config:   folder.FolderNameConfig{ProhibitSuffix: "_"},
		code:     code,
		expects: []testutil.Expect{
...
func emptyConfigCase(code string) folderTestCase {
	return newFolderCase(folderCaseConfig{
		...
		selector:   lint.Selector{PathRegexes: mustCompile(".*"), Kind: "folder"},
		config:     folder.FolderNameConfig{},
		code:       code,
		expectNone: true,
	})
...
func TestFolderName_CoreFunctionality(t *testing.T) {
	...
	runFolderNameCases(t, ctx, cases)
}
	...
	cases := []folderTestCase{
		{
			...
			checker: lint.WithSelector(
				lint.Selector{PathRegexes: mustCompile("^src/[^/]+$"), Kind: "folder"},
				folder.NewFolderNameCheck(folder.FolderNameConfig{Predicates: []string{"kebab"}}, checkCode, reporting.SevError),
			),
			expects: []testutil.Expect{
				{Code: checkCode, Msg: "does not satisfy predicate: kebab"},
			},
		},
		{
		...
		{
			name: "lowercase predicate fails on uppercase",
			buildTree: func(b *testutil.B) {
				b.Dir(srcDir, func(b *testutil.B) { b.Dir("API") })
			},
			checker: lint.WithSelector(
				lint.Selector{PathRegexes: mustCompile("^src/[^/]+$"), Kind: "folder"},
				folder.NewFolderNameCheck(folder.FolderNameConfig{Predicates: []string{"lowercase"}}, checkCode, reporting.SevError),
			),
			expects: []testutil.Expect{
				{Code: checkCode, Msg: "does not satisfy predicate: lowercase"},
			},
		},
		{
			name: "lowercase predicate passes on correct case",
			buildTree: func(b *testutil.B) {
				b.Dir(srcDir, func(b *testutil.B) { b.Dir("api") })
			},
			checker: lint.WithSelector(
		...
		{
			name: "predicate order determines reported reason (first failing predicate is reported)",
			buildTree: func(b *testutil.B) {
				b.Dir(srcDir, func(b *testutil.B) { b.Dir("Not-Kebab") })
			},
			checker: lint.WithSelector(
				lint.Selector{PathRegexes: mustCompile("^src/[^/]+$"), Kind: "folder"},
				folder.NewFolderNameCheck(folder.FolderNameConfig{Predicates: []string{"kebab", "lowercase"}}, checkCode, reporting.SevError),
			),
			expects: []testutil.Expect{
				{Code: checkCode, Msg: "does not satisfy predicate: kebab"},
			},
...
func TestFolderName_AllowDisallow(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			checker: lint.WithSelector(
				lint.Selector{PathRegexes: mustCompile(".*"), Kind: "folder"},
				folder.NewFolderNameCheck(
		...
		},
		{
			...
			checker: lint.WithSelector(
				lint.Selector{PathRegexes: mustCompile(`^[^/]+$`), Kind: "folder"}, // root-only
				folder.NewFolderNameCheck(
					folder.FolderNameConfig{Disallow: []string{`^model$`}},
					checkCode, reporting.SevError,
				),
...
func TestFolderName_Precedence(t *testing.T) {
	...
	cases := []folderTestCase{
		...
		},
		{
			...
			expects: []testutil.Expect{
				{Code: checkCode, Msg: "name 'Foo' does not satisfy predicate: lowercase"},
				{Code: checkCode, Msg: "name 'Foo' does not have required prefix 'lib-'"},
				{Code: checkCode, Msg: "name 'Foo' does not have required suffix '-svc'"},
			},
		},
	}
